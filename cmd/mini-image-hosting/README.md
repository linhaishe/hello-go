...
| 包         | 功能                                                        | 常见用途                                                     |
| ---------- | ----------------------------------------------------------- | ------------------------------------------------------------ |
| `context`  | 传递请求范围的上下文信息，并支持取消、超时和截止时间控制。  | HTTP 请求取消、数据库查询超时、协程间传递 trace ID。         |
| `log`      | 标准日志输出，默认带时间戳。                                | `log.Println` 记录运行信息；`log.Fatal` 输出错误后终止程序。 |
| `net/http` | 构建 HTTP 服务端和客户端。                                  | 创建 Web API、处理路由、发送 HTTP 请求。                     |
| `sync`     | 并发同步工具，防止多个 goroutine 同时访问共享数据导致问题。 | `sync.Mutex` 加锁；`sync.WaitGroup` 等待多个协程完成。       |

| 对比     | `fmt`                             | `log`                                                        |
| -------- | --------------------------------- | ------------------------------------------------------------ |
| 主要用途 | 格式化、打印、拼接字符串          | 输出程序运行日志                                             |
| 输出内容 | 默认只输出你提供的内容            | 默认附带日期和时间                                           |
| 输出位置 | 常用 `fmt.Println` 输出到标准输出 | 默认输出到标准错误                                           |
| 错误处理 | 不会终止程序                      | `log.Fatal` 会输出后调用 `os.Exit(1)`；`log.Panic` 会触发 panic |
| 常见场景 | 调试、命令行交互、构造字符串      | 记录启动信息、请求信息、错误和异常                           |
...
// 可能输出：
服务启动成功
2026/07/25 10:30:00 服务启动成功
```

一般来说：临时打印或字符串格式化用 `fmt`；需要保留时间、错误上下文、区分日志级别时用 `log`
...
我这次的排查，是按“先定位报错位置，再验证运行环境，最后把代码问题和系统限制分开看”来做的。

### 1. 先定位 `failed to create file` 是在哪一层抛出的
...
- `r.FormFile("file")` 已经拿到上传文件了
- 只是后面要写到本地文件时，`os.Create` 失败了
- 所以这个错误不是“表单没传过来”，而是“目标文件创建失败”

也就是说，排查重点从“上传请求有没有进来”转成了“写文件的那个目录和权限是否可用”。

---

### 2. 看代码用的是相对路径 uploads
...
- 你从哪个目录执行 `go run`，决定它会在哪个目录写文件
- 如果当前目录下没有 uploads，或者目录不可写，`os.Create` 就会失败

所以我不是直接猜“代码写错了”，而是先去验证：
...
- 代码本身可以编译
- 但当前环境的端口绑定被限制了
- 也就是说，服务没法在这个沙箱环境里正常监听 `8080`

所以这次的故障其实有两层：

1. 上传路径层面：
   - 目录不存在 / 不可写 → `failed to create file`

2. 运行环境层面：
   - 端口绑定受限 → `listen tcp :8080: bind: operation not permitted`

---

## 为什么这个排查思路是对的

因为它遵循了“从外到内、从现象到根因”的顺序：

1. 先看错误发生在哪个函数
2. 再看这个函数依赖什么外部条件
3. 再实际验证这些条件是否成立
4. 最后把“代码逻辑错误”和“运行环境限制”区分开

这很重要，否则你很容易把不同层面的错误混在一起。
...
- `failed to create file` 不是“上传表单坏了”，而是“保存文件时创建目标路径失败”
- 我通过查看代码、验证 uploads 目录、再验证服务启动环境，分别确认了：
  - 文件保存路径问题
  - 当前环境的端口限制问题

如果你愿意，我可以接着把这套排查步骤整理成一个“Go Web 上传服务排错 checklist”，你以后遇到类似问题就可以直接按这个顺序排。

“这里的根因，不是代码逻辑没写好，而是当前 VS Code/沙箱执行环境对文件创建和端口监听做了限制。”